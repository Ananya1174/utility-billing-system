...
class ConsumerDashboardControllerTest {
	...
	void summary_success() throws Exception {
		...
		context.setAuthentication(new UsernamePasswordAuthenticationToken("C1", null));
		SecurityContextHolder.setContext(context);

		Mockito.when(dashboardService.getDashboardSummary("C1"))
				.thenReturn(new ConsumerDashboardSummaryDto(1, 0, 2, 1, 500.0, 500.0, LocalDateTime.now()));

		mockMvc.perform(get("/consumers/dashboard/summary")).andExpect(status().isOk());
	}
}