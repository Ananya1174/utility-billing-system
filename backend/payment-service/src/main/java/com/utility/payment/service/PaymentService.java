...
public class PaymentService {
	...
	public PaymentResponse initiateOnline(InitiateOnlinePaymentRequest request) {
		...
		Payment payment = repository.findByBillIdAndStatus(request.billId(), PaymentStatus.INITIATED).orElseGet(() -> {
		...
		rabbitTemplate.convertAndSend(RabbitMQConfig.EXCHANGE, RabbitMQConfig.PAYMENT_OTP_KEY, event);
	...
	public List<PaymentResponse> getAllPayments() {
		...
		return payments.stream().map(payment -> {
			...
			return PaymentResponse.from(payment, bill, invoice != null ? invoice.getId() : null);
		}).toList();
	}
